  // After G86 threading cycle, output parameter summary if enabled
  if (cycle && cycle.command == 86 && getProperty("annotateThreadingSummary")) {
    writeComment("G86 Threading Cycle Parameter Table:");
    writeComment("X - Initial X position (absolute, radius/diameter mode)");
    writeComment("Z - Initial Z position (absolute)");
    writeComment("Q - Final X position (absolute, radius/diameter mode)");
    writeComment("R - Final Z position (absolute)");
    writeComment("K - Z coordinate for thread measurement (optional, used for thread repair)");
    writeComment("I - Thread depth (radius): positive = outside thread, negative = inside thread");
    writeComment("B - Pass depth (radius): positive = variable depth, negative = constant depth, 0 = error");
    writeComment("E - Minimum pass depth (used with B>0, optional, default 0)");
    writeComment("D - Safety distance X (radius): positive = round corner (G05), negative = square corner (G07), 0 = default");
    writeComment("L - Finish pass (radius): positive = same angle as other passes, negative = radial, 0 = repeat previous pass");
    writeComment("C - Thread pitch: positive = along taper, negative = along axis, 0 = error");
    writeComment("J - Thread exit Z: positive = direct exit, negative = exit to thread end then safety, 0 = blind thread");
    writeComment("A - Penetration angle (degrees): default 30, 0 = radial, negative = zig-zag, half tool angle = flank rubbing");
    writeComment("W - Spindle angular position: with K = repair, without K = thread start (multi-entry), optional");
    writeComment("V - Number of thread entries: optional, default 1, used with W for multi-entry threads");
    writeComment("M - Pitch increment/decrement per spindle revolution: positive = increment, negative = decrement, incompatible with K");
    writeComment("H - Entry type: 0 = radial, 1 = first flank, 2 = zig-zag first center, 3 = last flank, 4 = zig-zag last center, 5 = zig-zag flanks");
    writeComment("U - Partial thread repair/entry: 0 = normal, 1 = partial repair or cylinder entry (optional)");
  }
/**
...
properties = {
    annotateThreadingSummary: {
      title      : "Annotate G86 threading summary",
      description: "Add a summary block after G86 threading cycles explaining all parameters.",
      group      : "formats",
      type       : "boolean",
      value      : false,
      scope      : "post"
  ...
  annotateGMCodes: {
    title      : "Annotate G/M codes",
    description: "Add comments after each G/M code with meaning and section.",
    group      : "formats",
    type       : "boolean",
    value      : false,
    scope      : "post"
...
function writeBlock() {
  ...
  var codes = [];
  if (blockText) {
    // Find G and M codes in the block
    var codeRegex = /(G\d+|M\d+)/g;
    var match;
    while ((match = codeRegex.exec(blockText)) !== null) {
      codes.push(match[1]);
    }
  }
  ...
  if (getProperty("annotateGMCodes")) {
    if (codes.length > 0) {
      ...
      for (var i = 0; i < codes.length; i++) {
        var meaning = codeMeanings[codes[i]] || "";
        if (meaning) {
          comment += codes[i] + ": " + meaning;
        } else {
          comment += codes[i];
        }
        if (i < codes.length - 1) comment += ", ";
      }
    }
  }
  // Output block with annotation if enabled
  if (getProperty("showSequenceNumbers") == "true") {
    if (optionalSection) {
      if (blockText) {
        writeWords("/", "N" + sequenceNumber, blockText + comment);
      }
    } else {
      writeWords2("N" + sequenceNumber, blockText + comment);
    }
    sequenceNumber += getProperty("sequenceNumberIncrement");
  } else {
    if (optionalSection) {
      writeWords2("/", blockText + comment);
    } else {
      writeWords(blockText + comment);
    }
  }
}
